...
class Employee(models.Model):
    ...
    def get_user_employee_details(self):
        ...
        leaves_alloc_req = self.env['hr.holidays'].sudo().search_count([('state', 'in', ['confirm', 'validate1'])
                                                                                  , ('type', '=', 'add')])
        jum_pelatihan = self.env['hr.training.participant'].sudo().search_count([('name', '=', employee[0]['id'])])
        jum_perdin = self.env['hr.perdin.employee'].sudo().search_count([('employee_id', '=', employee[0]['id'])])
        jum_cuti = self.env['hr.holidays'].sudo().search_count([('employee_id', '=', employee[0]['id']), ('type', '!=', 'izin')])
        jum_izin = self.env['hr.holidays'].sudo().search_count([('employee_id', '=', employee[0]['id']), ('type', '=', 'izin')])
